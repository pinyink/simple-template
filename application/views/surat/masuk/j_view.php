<script type="text/javascript">
  ...
  $(document).ready(function() {
      ...
      table = $('#table').DataTable({ 
          ...
          "ajax": {
              ...
              // d.tgl = $('[name="tgl_bulan"]').val();
            }
              // "data": form_serialize_csrf($('#form-datatable').serialize())
          },
  ...
  function edit(id)
  {
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown)
      {
          notify('error', 'error', 'show data error');
      }
    });
  }
  <?php endif ?>
  ...
  function upload(id)
  {
    ...
    .done(function(data) {
      ...
      $.each(data, function(key, value) {
        ...
        var a = "<span id="+value.id+">"+value.deskripsi+" <a class='btn btn-xs btn-info btn-flat' href='"+u+value.path+"' target='_blank' data-toggle='tooltip' data-placement='top' title='lihat file'><i class='fa fa-download'></i></a></span> <br/><br/>";
        <?php endif ?>
        $('#list_scan').append(a);
      });
  ...
  $('#form_upload').submit(function(event) {
    ...
    .done(function(data) {
      if (data.stat == 0) {
        ...
        <?php endif ?>
        <?php if (isset($delete) and $delete == 0): ?>
        var a = "<span id="+data.id+">"+data.nama+" </button> <a class='btn btn-xs btn-info btn-flat' href='"+data.photo+"' target='_blank' data-toggle='tooltip' data-placement='top' title='lihat file'><i class='fa fa-download'></i></a><br/><br/></span>";
        <?php endif ?>
        $('#list_scan').append(a);
        $('#form_upload')[0].reset();
